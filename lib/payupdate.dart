...
class payUpdate extends StatefulWidget {
  final String name, cid, boever, amountpaid, paytype, status, jv, accver;
  final String id;

  payUpdate(this.id, this.name, this.cid, this.boever, this.amountpaid,
      this.paytype, this.status, this.jv, this.accver);

  @override
  State<payUpdate> createState() => _payUpdateState();
}

class _payUpdateState extends State<payUpdate> {
  ...
  Future updateDetail(String name, cid, boever) async {
    var url = Uri.parse("http://192.168.0.115/api/boea/payUpdate.php");
  ...
  Future getdata() async {
    var url = Uri.parse("http://localhost/api/delete.php");
    var rese = await http.post(url);
    var data = jsonDecode(rese.body);
    return data;
  }
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        errorText: _boever ? 'boea Can\'t empty' : null,
                      ),
                    ),
                    ButtonBar(
                            ...
                            onPressed: () {
                              setState(() {
                                ...
                                if (_name == false && _cid == false) {
                                  updateDetail(
                                      fname.text, ccid.text, bboever.text);
